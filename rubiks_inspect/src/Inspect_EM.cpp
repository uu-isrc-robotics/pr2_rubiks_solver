...
//			cv::floodFill(image_mat,cv::Point(x,y),CV_RGB(0,255,0),0,CV_RGB(5,5,5),CV_RGB(5,5,5), cv::FLOODFILL_FIXED_RANGE);
...
/////COM BLOCK MAY
		...
		CvMat samples = cvMat(54,3,CV_32FC1,cubie_colours.data);
		CvMat labls = cvMat(54,1,CV_32SC1,labels.data);
//		CvMat centrs = cvMat(6,3,CV_32FC1,centers.data);
	    ...
	    em_model.train( &samples, NULL, params, &labls );
		cv::Mat centers(em_model.get_means());
//		cvGet2D(em_model.get_means(),i,0)
//	    centers.
	    params.means = em_model.get_means();
	    std::cout << "Means: count=" << params.means->rows << "fields=" << params.means->cols << std::endl;
	    for (int i=0;i<6;i++) {
	    	std::cout << "Mean " << i << ":(" << cvGet2D(em_model.get_means(),i,0).val[0] << ", " << cvGet2D(em_model.get_means(),i,1).val[0] << ", " << cvGet2D(em_model.get_means(),i,2).val[0] << ")" << std::endl;
	    }
		...
		for (int i=0;i<6 ;i++) {
			for (int l=0;l<3;l++) {
				for (int k=0;k<3;k++) {
					...
					std::cout << (int)(chart[i*3 + l + step][k].r) << "\t" << (int)(chart[i*3 + l + step][k].g) << "\t" << (int)(chart[i*3 + l + step][k].b) << std::endl;

//					// The label colour applied
					chart[i*3 + l + step][k+4].r = cvGet2D(em_model.get_means(),labels.at<int>(j),2).val[0];//centers.ptr<float>(labels.at<int>(j))[2];
					chart[i*3 + l + step][k+4].g = cvGet2D(em_model.get_means(),labels.at<int>(j),1).val[0];//centers.ptr<float>(labels.at<int>(j))[1];
					chart[i*3 + l + step][k+4].b = cvGet2D(em_model.get_means(),labels.at<int>(j),0).val[0];//centers.ptr<float>(labels.at<int>(j))[0];
					j++;
				}
			}
...
//		cv::imwrite("debg2.png",centers);
		...
		std::string codes[]={"1","2","3","4","5","6","_","_","_","_","_"};
		description = "";
